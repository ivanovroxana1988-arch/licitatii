...
export async function GET() {
  ...
  const { data, error } = await service
    .from("associations")
    .select("id,name,leader_company_id,purpose,notes,created_at,updated_at,leader:companies!associations_leader_company_id_fkey(id,name,cui),members:association_members(company_id,role,responsibility,share_percent,is_leader,company:companies(id,name,cui))")
...
export async function POST(request: Request) {
  ...
  const service = createServiceClient();
  const { data: association, error } = await service.from("associations").insert(payload).select("*").single();
  if (error || !association) return NextResponse.json({ error: error?.message ?? "Nu am putut crea asocierea." }, { status: 500 });

  const memberRows = members.map((member) => ({ ...member, association_id: association.id }));
  ...
  return NextResponse.json({ association: full ?? association }, { status: 201 });
}
