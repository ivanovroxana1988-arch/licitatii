...
export default function AplicareForm({ token }: { token: string }) {
  ...
  return <div style={{ minHeight: "100vh" }}>
    ...
    <main style={{ maxWidth: 1080, margin: "0 auto", padding: 16, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 16 }}>
      ...
      <aside style={{ display: "grid", gap: 12, alignContent: "start" }}>
        <section style={card}><h2 style={h2}>Eligibilitate</h2>{eligibility?.details.map(d => <div key={d.eticheta} style={{ fontSize: 13, marginTop: 6, color: d.ok ? "#2e7d52" : "#b3261e" }}>{d.ok ? "✓" : "×"} {d.eticheta}</div>)}</section>
  ...
  function Documents() { return <div>{!readonly && <div style={{ display: "grid", gap: 8 }}><select value={uploadTip} onChange={e => setUploadTip(e.target.value as DocumentTip)} style={input}>{DOCUMENT_TYPES.map(t => <option key={t} value={t}>{documentTypeLabel(t)}</option>)}</select><input type="file" accept=".pdf,image/jpeg,image/png,image/webp" onChange={e => upload(e.target.files?.[0])} style={input} /></div>}<div style={{ display: "grid", gap: 8, marginTop: 10 }}>{payload.documente.map(d => <div key={d.id} style={row}><span>{d.nume_fisier}<br /><small>{documentTypeLabel(d.tip)}</small></span>{!readonly && <button onClick={() => delDoc(d.id)} style={danger}>Sterge</button>}</div>)}</div></div>; }
...
function Shell({ title, children }: { title: string; children: React.ReactNode }) { return <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: 20 }}><section style={card}><div style={mono}>FORMULAR FORMATOR</div><h1 style={h2}>{title}</h1>{children}</section></div>; }
